...
export const FormValidation = yup.object().shape({
    ...
    firstBuyerEmail: yup.string().email().required(REQUIRED),
    firstBuyerEmailVerification: yup.string().email().required(REQUIRED).oneOf([yup.ref('firstBuyerEmail'), null], "Email Addresses Must Match"),
    firstBuyerPhoneNumber: yup.string().required(REQUIRED).matches(phoneRegExp, 'This is not a valid phone number.'),
    ...
    dateHouseBuilt: yup.date().required(REQUIRED).typeError("Date Format Must Follow: MM/DD/YYYY"),

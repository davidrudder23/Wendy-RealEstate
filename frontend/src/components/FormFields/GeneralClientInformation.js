...
const GeneralClientInformation = ({errors, register, getValues, title }) => {
    const [count, setCount] = React.useState(
        getValues && getValues(`${title}.count`) ? getValues(`${title}.count`) : 1);

    // React.useEffect(() => {
    //     let newCount;
    //     if(getValues && (newCount = getValues(`${title}.count`) !== undefined)){
    //         setCount(newCount);
    //     }
    // }, [])

    console.log(getValues(`${title}.count`))
            ...
            fields.push(
                <S.FieldWrapper key={i} error={error}>
                        ...
                        <S.MultiContainer>
                            ...
                            required={true} 
                            getValues={getValues}/>
                            <InputField name={`${title}.${i}.lastName`} 
                            label="Last Name" 
